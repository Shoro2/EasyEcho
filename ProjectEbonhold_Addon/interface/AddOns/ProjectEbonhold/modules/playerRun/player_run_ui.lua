...
local function CreatePlayerRunFrame()
        ...
        if IsTransparentDesign() then
            ...
            if svc and svc.GetCurrentDifficulty then
                ...
                if diff <= 1 then
                    ...
                    GameTooltip:AddLine("No hardcore modifiers active.", 1, 1, 1, true)
                else
                    GameTooltip:SetText("Hardcore Mode " .. (diff - 1), 1, 0.3, 0.3)
                    GameTooltip:AddLine("Increased difficulty and rewards.", 1, 1, 1, true)
                end
            else
                GameTooltip:SetText("Normal Mode", 0.7, 0.7, 0.7)
            end
            ...
            local romanNumerals = { "I", "II", "III" }
            local isHardcore = tier > 1
            if isHardcore then
                GameTooltip:SetText("|cffFF2020Hardcore " .. (romanNumerals[tier - 1] or tier) .. "|r", 1, 0.2, 0.2)
            else
                GameTooltip:SetText("Hardcore", 0.8, 0.8, 0.8)
            end
            GameTooltip:AddLine("Click to open the Hardcore difficulty panel.", nil, nil, nil, true)
        end
        GameTooltip:Show()
    end)
    ...
    catchupHitbox:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetText("|cffffcc00Catch-Up Bonus|r", 1, 0.8, 0)
        GameTooltip:AddLine(" ")
        GameTooltip:AddLine("You are behind the progression curve. A " ..
            (playerRunFrame.currentCatchupPct or 0) ..
            "% bonus is applied on top of all other multipliers until you reach the previous patches' total Soul Ashes cap.",
            nil, nil, nil, true)
        GameTooltip:Show()
    end)
    ...
    if IsTransparentDesign() then
        empowermentHeader:SetSize(contentW + pad, echoesH)
        empowermentHeader:SetPoint("TOP", playerRunFrame, "BOTTOM", 0, -5)
    else
    ...
    if IsTransparentDesign() then
        if empowermentHeader.SetBackdrop then
            empowermentHeader:SetBackdrop({
                bgFile = "Interface\\Tooltips\\UI-Tooltip-Background",
                edgeFile = "Interface\\Tooltips\\UI-Tooltip-Background",
                tile = true,
                tileSize = 16,
            ...
            empowermentHeader:SetBackdropColor(0.1, 0.1, 0.1, 0.98)
            empowermentHeader:SetBackdropBorderColor(0, 0, 0, 1)
        end
    else
        local empowermentTexture = empowermentHeader:CreateTexture(nil, "BACKGROUND")
        empowermentTexture:SetTexture("Interface\\AddOns\\ProjectEbonhold\\assets\\texture_ui")
    ...
    local empowermentText = empowermentHeader:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    empowermentText:SetPoint("CENTER", empowermentHeader, "CENTER", 0, 0)
    empowermentText:SetText("Echoes")
    if IsTransparentDesign() then
        empowermentText:SetTextColor(1, 1, 1)
    end
    ...
    if IsTransparentDesign() then
        empowermentHeader:SetScript("OnEnter", function(self)
            if self.SetBackdropColor then
                ...
                self:SetBackdropColor(0.16, 0.42, 0.48, 0.98)
            end
            if empowermentText then empowermentText:SetTextColor(0.7, 0.95, 1) end
        end)
        empowermentHeader:SetScript("OnLeave", function(self)
            if self.SetBackdropColor then
                self:SetBackdropBorderColor(0, 0, 0, 1)
                self:SetBackdropColor(0.1, 0.1, 0.1, 0.98)
            end
            if empowermentText then empowermentText:SetTextColor(1, 1, 1) end
        end)
    end
    empowermentHeader:SetScript("OnClick", function() ToggleEmpowermentPanel() end)
    ...
    registerDragForward(empowermentHeader)
...
local function CreateEmpowermentFrame()
    ...
    browserButton:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
        GameTooltip:SetText("Echoes Browser", 1, 1, 1)
        GameTooltip:AddLine("Browse all available echoes", nil, nil, nil, true)
        GameTooltip:Show()
    end)
    ...
    browserButton:SetScript("OnClick", function()
        if ProjectEbonhold.PerkBrowser and ProjectEbonhold.PerkBrowser.Show then
    ...
    searchPlaceholder:SetText("Search echoes...")
...
local function UpdateEmpowermentDisplay(perksData)
        ...
        for i = 1, maxSlots do
            ...
            slotFrame:SetScript("OnEnter", function(self)
                ...
                if self.lockedPerkData then
                    ...
                    GameTooltip:AddLine(spellName or ("Spell " .. self.lockedPerkId), qData.color[1], qData.color[2],
                        qData.color[3])
                    ...
                    if lockedPerkDb and lockedPerkDb.families and #lockedPerkDb.families > 0 then
                        GameTooltip:AddLine(table.concat(lockedPerkDb.families, ", "), 0.4, 0.8, 1)
                    end
                    GameTooltip:AddLine(" ")
    ...
    for i, perkData in ipairs(perkList) do
        ...
        iconFrame:SetScript("OnEnter", function(self)
            ...
            GameTooltip:AddLine(pd.spellName or ("Spell " .. pd.spellId), qData.color[1], qData.color[2], qData.color[3])
            GameTooltip:AddLine(qData.name, 0.5, 0.5, 0.5)
...
local function UpdatePlayerRunData(data)
    ...
    if playerRunFrame.soulPointsText and data.soulPoints ~= nil then
        ...
        playerRunFrame.soulPointsText:SetText(string.format("|cffffffff%d|r", data.soulPoints))
    end
